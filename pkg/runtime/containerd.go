...
func (c *ContainerdRuntime) ReadIOPressure(container *container.ContainerInfo) (float64, float64, error) {
	...
	return c.cgroup.ReadIOPressure(cgroupPath)
}

func (c *ContainerdRuntime) DetectV1Throttle(container *container.ContainerInfo) (bool, float64, float64, float64, float64, float64, float64, error) {
	majMin, err := device.GetMajMin(c.config.DataMount)
	if err != nil {
	...
	ok, detail, err := c.cgroup.DetectV1Throttle(cgroupPath, majMin, time.Second, 0.9)
	if err != nil {
		return false, 0, 0, 0, 0, 0, 0, err
	}
