...
docker-buildx: build-multiarch ## 多架构Docker镜像构建
	...
	docker buildx build --platform linux/amd64 --build-arg BIN_NAME=kubediskguard-amd64 -t $(FULL_IMAGE_NAME)-amd64 --push .
	docker buildx build --platform linux/arm64 --build-arg BIN_NAME=kubediskguard-arm64 -t $(FULL_IMAGE_NAME)-arm64 --push .
